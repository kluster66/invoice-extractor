#!/usr/bin/env python3
"""
Script simple pour mettre à jour le code de la fonction Lambda (sans émojis)
"""
...
def update_lambda_simple():
    ...
    print(f"[OK] Package créé: {zip_path} ({file_size_mb:.2f} MB)")
    ...
    try:
        ...
        print(f"[OK] Code Lambda mis à jour avec succès!")
        print(f"  Version: {response['Version']}")
        print(f"  CodeSha256: {response['CodeSha256'][:20]}...")
        
    except Exception as e:
        print(f"[ERREUR] Erreur lors de la mise à jour du code: {e}")
        # Nettoyer
    ...
    try:
        ...
        print("[OK] Configuration Lambda mise à jour avec succès!")
    except Exception as e:
        print(f"[ERREUR] Erreur lors de la mise à jour de la configuration: {e}")
...
def test_lambda():
    ...
    try:
        ...
        if result.get('statusCode') == 500 and "'Records'" in str(result.get('body', '')):
            print("[OK] Test réussi! La fonction attend un événement S3 (champ 'Records')")
        else:
